...
def _compile_test_files(test_dir, libs_dir):
    ...
    subprocess.run([
        "gcc", "-o", os.path.join(test_dir, "main_with_rpath"),
        os.path.join(test_dir, "main.c"),
        "-L", libs_dir, "-lfoo", 
        f"-Wl,-rpath,$ORIGIN/libs"
    ], check=True)
    
    # Compile main without rpath
    subprocess.run([
        "gcc", "-o", os.path.join(test_dir, "main_no_rpath"),
        os.path.join(test_dir, "main.c"),
        "-L", libs_dir, "-lfoo"
    ], check=True)
    ...
    subprocess.run([
        "gcc", "-o", os.path.join(test_dir, "main_with_deps"),
        os.path.join(test_dir, "main_with_bar.c"),
        "-L", libs_dir, "-lfoo", "-lbar",
        f"-Wl,-rpath,$ORIGIN/libs"
    ], check=True)
...
def test_set_interpreter(arwen, bin_for_arwen, bin_for_patchelf):
    ...
    orig_interp, _, _ = run_command(["patchelf", "--print-interpreter", bin_for_patchelf])
    ...
    # Compare results
    patchelf_out, _, _ = run_command(["patchelf", "--print-interpreter", bin_for_patchelf])
...
def test_replace_needed(bin_for_arwen, bin_for_patchelf):
    ...
    run_command(["patchelf", "--replace-needed", "libtestbar.so", "libtestbar_new.so", bin_for_patchelf])
    run_command(["arwen", "elf", "replace-needed", bin_for_arwen, "libtestbar.so=libtestbar_new.so", ])
...
def test_force_rpath(bin_for_arwen, bin_for_patchelf):
    ...
    run_command(["patchelf", "--force-rpath", "--set-rpath", new_rpath, bin_for_patchelf])
...
def test_no_default_lib(bin_for_arwen, bin_for_patchelf):
    ...
    patchelf_out = subprocess.run(["greadelf", "-d", bin_for_patchelf], capture_output=True, text=True)
    arwen_out  = subprocess.run(["greadelf", "-d", bin_for_arwen], capture_output=True, text=True)

    assert "(FLAGS_1)            Flags: NODEFLIB" in patchelf_out.stdout, "no-default-lib flag not set"
    assert "(FLAGS_1)            Flags: NODEFLIB" in arwen_out.stdout, "no-default-lib flag not set"
...
def test_clear_symbol_versions(bin_for_arwen, bin_for_patchelf):
    ...
    patchelf_out = subprocess.run(["greadelf", "--syms", bin_for_patchelf], capture_output=True, text=True)
    arwen_out  = subprocess.run(["greadelf", "--syms", bin_for_arwen], capture_output=True, text=True)
    ...
    assert "chdir@GLIBC_2.2.5" not in patchelf_out.stdout, "symbol versions werent removed"
...
def test_clear_execstack(bin_for_arwen, bin_for_patchelf):
    ...
    # reading readelf output to check if the symbol version was removed
    patchelf_out = subprocess.run(["patchelf", "--print-execstack", bin_for_patchelf], capture_output=True, text=True)
    arwen_out = subprocess.run(["patchelf", "--print-execstack", bin_for_arwen], capture_output=True, text=True)
...
def test_rename_dynamic_symbols(bin_for_arwen, bin_for_patchelf, tmp_files):
    ...
    run_command(["arwen", "elf", "rename-dynamic-symbols", bin_for_arwen, "chdir=chdir_new"])
    
    # reading readelf output to check if the symbol was renamed
    patchelf_out = subprocess.run(["nm", "-D", bin_for_patchelf], capture_output=True, text=True)
    arwen_out  = subprocess.run(["nm", "-D", bin_for_arwen], capture_output=True, text=True)
    
    assert "chdir@GLIBC_2.2.5" not in patchelf_out.stdout, "symbol versions werent renamed"
    assert "chdir@GLIBC_2.2.5" not in arwen_out.stdout, "symbol versions werent renamed"

    assert "chdir_new@GLIBC_2.2.5" in patchelf_out.stdout, "symbol versions werent renamed"
