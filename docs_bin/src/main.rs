...
/// Generates documentation for the Arwen CLI by:
/// - Loading the clap command from thearwen crate
...
fn arguments(options: &[&clap::Arg], parents: &[String]) -> String {
    ...
    for opt in options {
        ...
        let long_name = if let Some(long) = opt.get_long() {
            ...
            eprintln!("value names: {:?}", value_names);
        ...
        if !opt.get_default_values().is_empty() {
            writeln!(
                ...
                opt.get_default_values()
                    .iter()
                    .map(|opt_v| opt_v.to_string_lossy().to_string())
                    .join(", ")
            )
            .unwrap();
        }
